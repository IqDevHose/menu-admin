...
const Home = () => {
  ...
  const customerReviewQuery = useQuery({
    ...
    queryFn: async () => {
      const customerReview = await axiosInstance.get(`/customer-review`);
      return customerReview.data;
    },
  });
  const statsData = {
          ...
          <Col xs={24} sm={12} lg={6}>
            <Card hoverable>
              <Statistic
                ...
                value={query.data.totalCategories}
                valueStyle={{ color: "#3f8600" }}
              />
            </Card>
          </Col>
      ...
      <div className="w-80 pl-4">
        <Card title="Recent Activity" className="mb-6">
          <List
            ...
            dataSource={
              customerReviewQuery?.data?.items?.slice(
                0,
                4
              ) as CustomerReviewType[]
            }
            renderItem={(item) => (
              <List.Item>
                <List.Item.Meta
